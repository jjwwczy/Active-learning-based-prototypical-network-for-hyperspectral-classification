...
class PairDataset(torch.utils.data.Dataset):#需要继承data.Dataset
    def __init__(self,Datapath1,Datapath2,Labelpath,trans):
        ...
        self.data=np.load(Datadir+'RequestX.npy')
...
Datadir='./DataArray/'
ytrain=np.load(Datadir+'RequestY.npy')
datalist1=[]
datalist2=[]
labellist=[]
...
if TRAIN:
    ...
    Datapath=Datadir+'RequestX.npy'
    Labelpath=Datadir+'RequestY.npy'
    train(model,Datapath1,Datapath2,PairLabelpath,Datapath,Labelpath,trans,epochs=epoch)
train_end_time=time.time()
#Testing
model.load_state_dict(torch.load('model.pth'))
Datapath=Datadir + 'Xtest.npy'
Labelpath=Datadir + 'ytest.npy'
